...
In R, the value of the slope coefficient $b$ 
...
2. an estimate of the standard deviation $\sigma_b$ of 
that estimated factor ("`Std. Error`") is 
`r (sigma_b <- round(sL$coefficient[2], 5))`, and
...
then the projection
of $y$ is simply $$90*`r A` = `r (yhat <- round(90*b, 0))`$$
The risk that we are wrong under our model
$$y = ax + \sqrt{x}e$$
arises from an inacurate estimate of the parameter $b$ 
("parameter risk")
...
$$ std(b \cdot c) = std(b) \cdot = \sigma_b \cdot c$$

#### Parameter risk

In the triangle $(x,y)$ the values of $y$
are considered random variables 
*conditional on the given values of $x$* and
the values of $x$ are considered "constants".
So the estimated link ratio $b$
is considered a random variable
and the the new value $x=90$
is considered a constant.

Using Trick 1 with $b=`r b`$ and $c=90$
the parameter risk of the projection 
$$ `r b` \cdot 90$$

is
$$ \sigma_b \cdot 90 = (`r sigma_b`)(90) = `r (parr <- 90*sigma_b)`$$
...
but that's the jist of the argument)

In other words,
don't be surprised if the projection $y = `r yhat`$ 
from the new value of $x = 90$
based on the weighted average development factor `r b` is less than 
...
is estimated from this slightly less-slim triangle
```{r}
trisave <- tri
kable_styling(kable(tri, row.names = TRUE), full_width = FALSE)
tri <- cbind(tri,
